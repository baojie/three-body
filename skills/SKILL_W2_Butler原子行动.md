...
| D2 `scene-scan` | 50 | 10 | queue.md，名场面候选 10–20 条 |
...
主动识别三部曲中的**名场面**（经典场景/决定性时刻），写入 queue.md P1。

名场面不会被 discover_wanted 发现（无破碎 wikilink），也不会被 discover_corpus 命中（场景词非高频实体名）。需人工叙事记忆驱动，corpus 验证确认。

**步骤**：

```
1. 从三个维度各列候选（共 20–30 个）：
   - 三体I：物理危机场景 / 三体游戏关卡 / 红岸基地时刻
   - 三体II：面壁计划关键场景 / 黑暗战役 / 舰队逃亡
   - 三体III：威慑纪元对峙 / 降维打击 / 宇宙末年

2. 排除已建页面：
   ls wiki/public/pages/*.md | xargs -I{} basename {} .md | sort

3. 对每个候选用 corpus_search 验证：
   python3 wiki/scripts/butler/corpus_search.py "场景核心词" --max 5
   → ≥ 1 命中：确认，记录 PN，写入 P1
   → 0 命中：换关键词重试，仍无则跳过

4. 写入 queue.md P1（每批 10–20 条）：
   - [ ] P1 create | 叶文洁发射信号 | 名场面：叶文洁向三体文明发送第一条信号（1-04-043）
   - [ ] P1 create | 台球桌实验 | 名场面：丁仪用台球桌演示物理规律被智子干扰（1-06-013）
```

**优先纳入**：
- 改变故事走向的决定性行动（章北海逃亡、程心接剑）
- 宇宙观冲击性场景（降维打击、四维空间审判）
- 高情感浓度时刻（云天明的星星、关一帆小宇宙）
- 反复被读者引用/讨论的段落（罗辑咒语、叶文洁山顶独白）

**diff 上限**：只改 queue.md，≤ 25 行
...
名场面（`tags` 含 `名场面`）是**人物 × 主题 × 情节**的交叉点——即"这个角色的这次行动，同时推动了情节、揭示了人物，并承载了作品的核心主题之一"。
...
**详版（5–7节）**：事件跨越多章、有准备阶段、技术细节重要，或道德复杂性高
```
## 动机与背景        ← 为什么这个角色会做这件事（长期布局/历史压力）
## 准备：XXX的构思  ← 行动的关键设计，原文技术细节（如：陨石子弹的制造）
## 执行：场景名     ← 事件本体，按时间顺序，含 PN 引文
## 为何XXX          ← 事件的内在逻辑（为何不可见/不可追/为何有效），深化读者理解
## 象征意义         ← 隐喻层：该场景对作品主题的贡献
## 叙事地位         ← 在三部曲全局中的位置（承上：触发了什么；启下：导致了什么）
## 相关词条
```

选哪种结构：情节复杂 + 有技术设计细节 + 道德模糊 → 用详版；短小精悍的决定性瞬间 → 用简版。
...
type: event
label: 场景名
aliases: [别名, 原文标志性短语]   # 短语用于 wikify_chapters 锚点
tags: [名场面, 三体I|II|III, 主要人物名, 相关概念标签]
description: 一句话，≤50字，含书册位置+核心动作+意义
books: [三体II]
quality: featured
---
```
...
```bash
# 1. 用场景的关键物/动词定位起点
python3 wiki/scripts/butler/corpus_search.py "关键物或动词" --max 15

# 2. 记录起点 PN（如 2-24-060），在原文 txt 中向后读 20–30 段
grep -A 30 "2-24-060" corpus/utf8/三体II.txt
...
1. **场景经过节要像小说摘要，不是事件清单**：还原现场感，按角色视角叙述，不是列要点
2. **技术细节要有原文支撑**：陨石怎么制造 / 水滴怎么移动 / 飞刃怎么布设 — 都必须有 PN
3. **象征意义要从细节往上走**：先说"这里发生了什么"，再说"这象征着什么"，不能只讲主题
4. **叙事地位要写因果链**：这个事件发生前，世界是什么状态？发生后，什么改变了？
5. **禁止主观评价**：不写"精彩"/"震撼"/"令人叹服"——让内容自己说话

**名场面页 checklist**（自评时用）：
- [ ] PN ≥ 3 条，均来自 corpus_search 验证，不是猜测
- [ ] `## 场景经过`（或执行节）有具体细节：时间/地点/动作/对话，不是泛泛概括
- [ ] `## 象征意义` 有分析性段落，不是事实罗列，要有"隐喻"或"对照"的明确表述
- [ ] `## 叙事地位` 点明在三部曲全局中的位置（前因后果各一句）
- [ ] aliases 包含原文中出现的标志性短语（供 wikify_chapters 锚点法使用）
- [ ] `:::seealso` 块已填写（≥ 3 条）
- [ ] `## 相关词条` ≥ 4 条，含场景中的核心人物和概念
...
- [W6 编辑原则](SKILL_W6_编辑原则.md) — 废话三类定义 + 内容写入前的检测测试
