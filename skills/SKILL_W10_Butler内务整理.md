...
| H26 | scene-discover | 150 | 3 | 扫描角色弧线与情节节点，发现未建的名场面（事件）词条 | H-P3（每37轮）|
...
### H26 · scene-discover（名场面发现扫描）

`discover_wanted.py` 通过词频扫描发现高频实体词，但**无法发现叙事性事件（名场面）**——因为名场面往往没有固定的高频短语出现，而是跨越多个段落的情节交汇点。H26 是专门补足这一盲区的扫描任务。

#### 为什么 pipeline 无法自动发现名场面

- 词频扫描只识别**名词性实体**（人名/地名/术语），不识别**事件**
- 名场面的"名称"往往是读者/评论者的总结，而非原文固定词语（如"陨石暗杀"原文无此词）
- 情节的重要性体现在**多段落叙事密度**，不体现在单词出现频率

#### H26 扫描方法：角色弧线审计

对每位**主要人物**逐一列举其核心行动节点，判断是否已有对应词条：

```
主要人物：叶文洁 / 罗辑 / 章北海 / 程心 / 云天明 / 维德 / 托马斯·韦德 / 申玉菲 / 史强 / 汪淼…

对每位人物，问三个问题：
  1. 这个角色的最重要一次"行动/抉择"是什么？
  2. 读者最容易记住的"情节画面"是哪几个？
  3. 对整个故事走向影响最大的一个节点是什么？

检查对应词条是否存在 → 不存在则写入 queue.md P2
```
...
| 信号 | 说明 | 示例 |
|------|------|------|
| 情节密度 | 单段情节跨越 5+ 个连续 PN 段落 | 章北海黄河站暗杀（约20段） |
| 转折性 | 该事件直接导致世界状态改变 | 叶文洁发射 → 三体文明入侵开始 |
| 道德复杂性 | 事件迫使角色做出两难选择，引发读者讨论 | 程心收回剑柄 → 威慑失效 |
| 读者记忆性 | 情节高度形象化，读者多年后仍记得细节 | 水滴消灭舰队 / 二向箔展开 |
| 象征密度 | 单场景同时承载多层主题意义 | 地球炉大会场景（冬眠者与新人类的冲突） |

#### 检查已有词条的方法

```bash
# 搜索该场景的相关词汇，判断是否已建页
python3 wiki/scripts/butler/corpus_search.py "黄河站" --max 5
ls wiki/public/pages/ | grep "章北海"

# 查看角色页面的相关词条节，已链接 = 已建
grep "陨石" wiki/public/pages/章北海.md
```
...
```
H26 scene-discover：审计角色 N 位
发现未建词条：
  - [P2] 程心收回剑柄（event，三体III，主要人物：程心）— 触发：转折性+道德复杂性
  - [P2] 地球炉大会（event，三体III，主要人物：程心/庄颜）— 触发：情节密度+象征密度
  - [P3] 汪淼进入三体游戏（event，三体I，主要人物：汪淼）— 触发：读者记忆性
已有词条（跳过）：
  - 章北海逃亡 ✓ / 章北海陨石暗杀 ✓ / 叶文洁发射 ✓
```

**WU**：150（读角色弧线 + corpus 验证 + 写入队列）
