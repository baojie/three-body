---
name: skill-butler-1
description: 三体 Wiki Butler 的任务选取与探索策略。三食物源（broken links、语料频率、队列手动条目）、三队列系统（content/housekeeping）、trail/explore 动态配比。每轮按本 skill 选出目标，交由 W2 执行。
---
...
```bash
# 频率扫描：发现语料中高频但未建页的实体
python3 wiki/scripts/butler/discover_corpus.py --top 30 --min-freq 3
```
...
5. 否则（p1_pending 和 p2_pending 均为空，或 round % 11 == 0）→ 补货：
   a. 运行 discover_wanted.py --top 20（自动 fallback 到语料扫描）
   b. 运行 discover_corpus.py --top 20 --min-freq 3（补充语料候选）
...
每轮可 **discover 1–5 条候选** 写入 queue.md，但本轮只执行 1 条（或 1 批同类任务）。

- **P1 条件**：broken link 被引用 ≥ 3 次，或语料频率 ≥ 20 次的舰船/核心概念，或主角关联词条
- **P2 条件**：broken link 被引用 1–2 次，或语料频率 5–19 次的候选
- **P3 条件**：全局扫描类探索任务
...
# 场景2：无 broken links（自动语料模式）
python3 wiki/scripts/butler/discover_wanted.py --top 15
# 输出 → 85x 星环号[舰船] → 写入 P1

# 场景3：直接语料扫描（补充）
python3 wiki/scripts/butler/discover_corpus.py --top 20 --min-freq 3
```

转入 queue.md（追加到 P1/P2 段落末尾）：
```markdown
- [ ] P1 create | 星环号 | 语料85次，三体III核心舰船
- [ ] P1 create | 自然选择号 | 语料59次，章北海所在战舰
- [ ] P2 create | 群星计划 | 语料12次，威慑纪元太空移民计划
```
