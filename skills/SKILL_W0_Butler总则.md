...
1. **批量**：每轮目标 1000 WU，batch_n=ceil(1000/WU) 页（enrich 50WU→20页，create 100WU→10页）；单页 diff ≤ 60 行
...
启动
  ...
  │
  ├─Step 3  周期任务检查
  │           round % 29 == 0 → W5 强制反思（本轮只做反思；若为 Dream Round，先运行 cleanup_queue.py）
  │           round % 17 == 0 → /wiki 发布
  │           round % 29 == 0 → D1 discover（--top 60）+ H10 housekeeping-scan（与 W5 同轮触发，W5 优先）
...
| 周期 | 条件 | 任务 |
|------|------|------|
| 每 29 轮 | `round % 29 == 0` | D1 discover + H10 housekeeping-scan（与 W5 同轮；W5 优先，同轮先 W5 再 discover） |
