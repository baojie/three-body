...
- 未处理的批次写入 `wiki/logs/butler/queue.md`（P2 优先级，格式：`- [ ] [P2] beaver <source-slug> 批次<N> create`）
- 进度文件存放在 `wiki/logs/beaver/<source-slug>/`

### 断点续传

下次调用 `/beaver --loop <文件路径>` 时，自动读取 `progress.md`，从上次中断处继续：
